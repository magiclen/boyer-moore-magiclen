...
fn data_input_from_file() {
    ...
    for _ in 0..pattern_count {
        ...
        for _ in 0..text_count {
            ...
            assert_eq!(answer.clone(), bm.find_all_in_u8_slice(&text));
            assert_eq!(answer.iter().map(|&i| i).rev().collect::<Vec<usize>>(), bm.rfind_all_in_u8_slice(&text));

            let bm = BMLatin1::from_str(&pattern).unwrap();

            assert_eq!(answer.clone(), bm.find_all_in_str(&text));
            assert_eq!(answer.iter().map(|&i| i).rev().collect::<Vec<usize>>(), bm.rfind_all_in_str(&text));
